...
class CrudCode extends CCodeModel
{
	...
	public function generateActiveLabel($modelClass, $column, $classArr = '')
	{
		$classArr = var_export($classArr, true);

		return "\$form->labelEx(\$model,'{$column->name}',$classArr)";
	}

	public function generateActiveField($modelClass, $column, $classArr = '')
	{
		$classArr = var_export($classArr, true);
		...
		elseif (stripos($column->dbType, 'text') !== false)
			return "\$form->textArea(\$model,'{$column->name}',$classArr))";
		else {
			if (preg_match('/^(password|pass|passwd|passcode)$/i', $column->name))
			...
			if ($column->type !== 'string' || $column->size === null)
				return "\$form->{$inputField}(\$model,'{$column->name}',$classArr)";
			else {
				if (($size = $maxLength = $column->size) > 60)
					$size = 60;
				return "\$form->{$inputField}(\$model,'{$column->name}',$classArr)";
