...
class Utils
{
    ...
    public static function converstDate($inputFomat, $targetFomat, $inputDate)
    {
        ...
        // $targetFomat = 'Y-m-d'; // output format string
        if (empty($inputDate)) return false;

        $date = DateTime::createFromFormat($inputFomat, $inputDate);

        $new_date_str = $date->format($targetFomat);
